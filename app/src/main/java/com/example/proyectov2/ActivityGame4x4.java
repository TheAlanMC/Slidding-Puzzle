...
public class ActivityGame4x4 extends AppCompatActivity {
    ...
    String patron="123456789101112131415X";
    String nombre;
    String time;
    Boolean estado;
    int seconds=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv1=findViewById(R.id.tva1);
        tv2=findViewById(R.id.tva2);
        tv3=findViewById(R.id.tva3);
        tv4=findViewById(R.id.tva4);
        tv5=findViewById(R.id.tva5);
        tv6=findViewById(R.id.tva6);
        tv7=findViewById(R.id.tva7);
        tv8=findViewById(R.id.tva8);
        tv9=findViewById(R.id.tva9);
        tv10=findViewById(R.id.tva10);
        tv11=findViewById(R.id.tva11);
        tv12=findViewById(R.id.tva12);
        tv13=findViewById(R.id.tva13);
        tv14=findViewById(R.id.tva14);
        tv15=findViewById(R.id.tva15);
        tv16=findViewById(R.id.tva16);
        ...
        for(int i=0;i<vTv.size();i++){
            ...
            vTv.elementAt(ii).setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    for(int j=0;j<4;j++) {
                        for (int k = 0; k < 4; k++) {
                            if (((Math.abs(jj-j)+Math.abs(kk-k)) == 1) && vTv.elementAt(j*4+k).getText().toString().equals("X")) {
                                swap(vTv.elementAt(ii), vTv.elementAt(j*4+k));
                                steps.add(ii);
                                moves+=1;
            ...
            public void onClick(View view) {
                if (estado){
                    ...
                    for (int i=0; i<100; i++){
                        Boolean flag = true;
                        while (flag){
                            int ii=(int) (Math.random()*16);
                            int jj=ii/4;
                            int kk=ii%4;
                            for(int j=0;j<4;j++) {
                                for (int k = 0; k < 4; k++) {
                                    if (last!=ii && ((Math.abs(jj-j)+Math.abs(kk-k)) == 1) && vTv.elementAt(j*4+k).getText().toString().equals("X")) {
                                        swap(vTv.elementAt(ii), vTv.elementAt(j*4+k));
                                        steps.add(ii);
                                        flag=false;
                                        last=last2;
                    ...
                    myHandler.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            if(steps.size()>=2)
                                ...
                                swap(vTv.elementAt(steps.get(steps.size()-1)),vTv.elementAt(15));
    ...
    private void swap (TextView tva, TextView tvb){
        ...
        final ColorStateList colorFont=tva.getTextColors();
        tva.setText(tvb.getText().toString());
        tva.setBackground(tvb.getBackground());
        tva.setTextColor(tvb.getTextColors());
        tvb.setText(txtTmp);
        tvb.setBackground(colorTmp);
        tvb.setTextColor(colorFont);
    }
    ...
    private void new_game(){
        ...
        int flag=0;
        for(int i=0;i<vTv.size()-1;i++){
            vTv.elementAt(i).setTextColor(Color.parseColor("#CFA240"));
            vTv.elementAt(i).setText(Integer.toString(i+1));
            if(i%4==0){
                flag=1-flag;
            }
            if(i%2==0 && flag==1)
                vTv.elementAt(i).setBackgroundColor(Color.parseColor("#D00016"));
            else if (i%2==1 && flag==1)
                vTv.elementAt(i).setBackgroundColor(Color.parseColor("#D0D4D6"));
            else if(i%2==0 && flag==0)
                vTv.elementAt(i).setBackgroundColor(Color.parseColor("#D0D4D6"));
            else
                vTv.elementAt(i).setBackgroundColor(Color.parseColor("#D00016"));
        }
        vTv.elementAt(15).setTextColor(Color.parseColor("#FFFFFF"));
        vTv.elementAt(15).setText("X");
        vTv.elementAt(15).setBackgroundColor(Color.parseColor("#FFFFFF"));
    }
